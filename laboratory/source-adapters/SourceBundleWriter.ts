...
export class SourceBundleWriter {
    ...
    async write(bundle: GitHubSourceBundle): Promise<void> {

        const bundlePath = `./sources/research/${bundle.sourceId}`;
            ...
            JSON.stringify(
                {
                    ...
                    hasImplementation: evidence.hasImplementation,
                    hasTests: evidence.hasTests,
                    hasInvariants: evidence.hasInvariants,
                    hasCoverage: evidence.hasCoverage,
                    hasCitation: evidence.hasCitation,
...
${bundle.protocols.map(protocol => `- ${protocol}`).join("\n")}

## Capabilities

${bundle.capabilities.map(capability => `- ${capability}`).join("\n")}

## Claims

${bundle.claims.map(claim => `- ${claim}`).join("\n")}
`
        );
    ...
    private async updateManifest(bundle: GitHubSourceBundle): Promise<void> {
        ...
        const entry = {
            sourceId: bundle.sourceId,
            path: `./sources/research/${bundle.sourceId}/source.json`,
            evidencePath: `./sources/research/${bundle.sourceId}/evidence.json`,
            enabled: false
        };

        manifest.sources = manifest.sources.filter(
            (source: any) => source.sourceId !== bundle.sourceId
        );
